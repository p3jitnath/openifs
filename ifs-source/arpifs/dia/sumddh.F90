...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV, &
 ...
 & YDCSGEOM=>YDGEOMETRY%YRCSGEOM, &
  & YDCSGEOM_NB=>YDGEOMETRY%YRCSGEOM_NB,  YDSPGEOM=>YDGEOMETRY%YSPGEOM, YDMDDH=>YDML_DIAG%YRMDDH,  &
  & YDLDDH=>YDML_DIAG%YRLDDH)

ASSOCIATE(NDGENG=>YDDIM%NDGENG, NDGENL=>YDDIM%NDGENL, NDGSAG=>YDDIM%NDGSAG, &
 & NDGSAL=>YDDIM%NDGSAL, NDGUXL=>YDDIM%NDGUXL, NDLON=>YDDIM%NDLON, &
 & NDGUNL=>YDGEOMETRY%YRDIM%NDGUNL, NDLUXG=>YDDIM%NDLUXG, &
 & NGPTOT=>YDGEM%NGPTOT, NSTAGP=>YDGEM%NSTAGP, &
 & NTSTAGP=>YDGEM%NTSTAGP, &
 & LHDDOP=>YDLDDH%LHDDOP, LHDGLB=>YDLDDH%LHDGLB, LHDLIST=>YDLDDH%LHDLIST, &
 & LHDZON=>YDLDDH%LHDZON, LSDDH=>YDLDDH%LSDDH, &
 & BDEDDH=>YDMDDH%BDEDDH, FNODDH=>YDMDDH%FNODDH, HDSF=>YDMDDH%HDSF, &
 & HDSFDU=>YDMDDH%HDSFDU, HDSFGL=>YDMDDH%HDSFGL, HDSFLA=>YDMDDH%HDSFLA, &
 & NDDHI=>YDMDDH%NDDHI, NDDHLA=>YDMDDH%NDDHLA, NDDHPU=>YDMDDH%NDDHPU, &
 & NDHBPU=>YDMDDH%NDHBPU, NDHBPX=>YDMDDH%NDHBPX, NDHDDX=>YDMDDH%NDHDDX, &
 & NDHIDH=>YDMDDH%NDHIDH, NDHKD=>YDMDDH%NDHKD, NDHNOM=>YDMDDH%NDHNOM, &
 & NDHNPU=>YDMDDH%NDHNPU, NLRDDH=>YDMDDH%NLRDDH, NLXDDH=>YDMDDH%NLXDDH, &
 & NURDDH=>YDMDDH%NURDDH, NUXDDH=>YDMDDH%NUXDDH, &
 & LSPLITLAT=>YDMP%LSPLITLAT, MYLATS=>YDMP%MYLATS, NFRSTLAT=>YDMP%NFRSTLAT, &
 & NLSTLAT=>YDMP%NLSTLAT, NONL=>YDMP%NONL, NPTRFLOFF=>YDMP%NPTRFLOFF, &
 & NPTRFRSTLAT=>YDMP%NPTRFRSTLAT, NPTRLSTLAT=>YDMP%NPTRLSTLAT, NSTA=>YDMP%NSTA)
!     ------------------------------------------------------------------
...
!     Zonal stripe index for each gridpoint :
      NDDHLA(JLON+ISTAGP(JGL)-1)=&
       & NINT((1.0_JPRB-ZGEMU(JLON+ISTAGP(JGL)-1))&
       & *REAL(NDHKD,JPRB)*0.5_JPRB+0.5_JPRB)  
