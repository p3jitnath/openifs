...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGFLPF(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YDML_GCONF%YGFL%NDIMSLP) 
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, YDMP=>YDGEOMETRY%YRMP, &
 ...
 & YDGSGEOM=>YDGEOMETRY%YRGSGEOM(KIBL), YDSPGEOM=>YDGEOMETRY%YSPGEOM, &
 & YDDYN=>YDML_DYN%YRDYN,YDDYNA=>YDML_DYN%YRDYNA,YDPTRSLB1=>YDML_DYN%YRPTRSLB1,YGFL=>YDML_GCONF%YGFL, &
 & YDDIMF=>YDML_GCONF%YRDIMF,YDTLSCAW=>YDML_DYN%YYTLSCAW,YDTRSCAW=>YDML_DYN%YYTRSCAW)

ASSOCIATE(NUMFLDS=>YGFL%NUMFLDS, YCOMP=>YGFL%YCOMP, &
 & NUMFLDS_SL1=>YGFL%NUMFLDS_SL1, NUMFLDS_SPL=>YGFL%NUMFLDS_SPL, &
 & NPROMA=>YDDIM%NPROMA, &
 & NFTHER=>YDDIMF%NFTHER, &
 & NFLEN=>YDDIMV%NFLEN, NFLEVG=>YDDIMV%NFLEVG, NFLSA=>YDDIMV%NFLSA, &
 & RRINTOPT=>YDDYN%RRINTOPT, LQMHP=>YDDYN%LQMHP, LQMHPD=>YDDYN%LQMHPD, &
 & LQMHT=>YDDYN%LQMHT, LQMHVD=>YDDYN%LQMHVD, LQMHW=>YDDYN%LQMHW, &
 & LQMP=>YDDYN%LQMP, LQMPD=>YDDYN%LQMPD, LQMT=>YDDYN%LQMT, LQMVD=>YDDYN%LQMVD, &
 & LQMW=>YDDYN%LQMW, LSPLTHOIGFL=>YDDYN%LSPLTHOIGFL,LRHS_CURV=>YDDYN%LRHS_CURV, &
 & LVWENO_W=>YDDYN%LVWENO_W,LVWENO_T=>YDDYN%LVWENO_T,LVWENO_SP=>YDDYN%LVWENO_SP, &
 & LVWENO_SPD=>YDDYN%LVWENO_SPD,LVWENO_SVD=>YDDYN%LVWENO_SVD, &
 & WENO_ALPHA_W=>YDDYN%WENO_ALPHA_W,WENO_ALPHA_T=>YDDYN%WENO_ALPHA_T, &
 & WENO_ALPHA_SP=>YDDYN%WENO_ALPHA_SP, &
 & WENO_ALPHA_SPD=>YDDYN%WENO_ALPHA_SPD,WENO_ALPHA_SVD=>YDDYN%WENO_ALPHA_SVD, &
 & NCURRENT_ITER=>YDDYN%NCURRENT_ITER, NSITER=>YDDYN%NSITER, &
 & NSLDIMK=>YDDYN%NSLDIMK, NSPDLAG=>YDDYN%NSPDLAG, NSPLTHOI=>YDDYN%NSPLTHOI, &
 & NSVDLAG=>YDDYN%NSVDLAG, NTLAG=>YDDYN%NTLAG, NVLAG=>YDDYN%NVLAG, &
 & NWLAG=>YDDYN%NWLAG, LSLHDHEAT=>YDDYN%LSLHDHEAT, &
 & YGP=>YDGMV%YGP, &
 & LRSIDDH=>YDLDDH%LRSIDDH, LRSLDDH=>YDLDDH%LRSLDDH, &
 & MSIDDH_PD0=>YDMDDH%MSIDDH_PD0, MSIDDH_T0=>YDMDDH%MSIDDH_T0, &
 & MSIDDH_U0=>YDMDDH%MSIDDH_U0, MSIDDH_V0=>YDMDDH%MSIDDH_V0, &
 & MSIDDH_VD0=>YDMDDH%MSIDDH_VD0, MSLDDH_NHX=>YDMDDH%MSLDDH_NHX, &
 ...
 & MSLB1GFLP9=>YDPTRSLB1%MSLB1GFLP9, MSLB1GFLSP9=>YDPTRSLB1%MSLB1GFLSP9, &
 & MSLB1GFLSPF9=>YDPTRSLB1%MSLB1GFLSPF9, MSLB1GWS9=>YDPTRSLB1%MSLB1GWS9, &
 ...
 & MSLB1C9_NL=>YDPTRSLB1%MSLB1C9_NL, MSLB1PD9_NL=>YDPTRSLB1%MSLB1PD9_NL, &
 & MSLB1T9_NL=>YDPTRSLB1%MSLB1T9_NL, MSLB1U9_NL=>YDPTRSLB1%MSLB1U9_NL, &
...
IF(NWLAG >= 3) THEN
  ...
  IF (LD2TLFF1) THEN
    ...
    ! Conversion
    DO JLEV=1,NFLEVG
      PUFZ(KST:KPROF,JLEV)=-ZGMVFU(KST:KPROF,JLEV)*YDGSGEOM%GESLO(KST:KPROF)  &
       &                   +ZGMVFV(KST:KPROF,JLEV)*YDGSGEOM%GECLO(KST:KPROF)
      PVFZ(KST:KPROF,JLEV)=-ZGMVFU(KST:KPROF,JLEV)                            &
       &      *YDGSGEOM%GECLO(KST:KPROF)*YDGSGEOM%GEMU(KST:KPROF)             &
       &                   -ZGMVFV(KST:KPROF,JLEV)                            &
       &      *YDGSGEOM%GESLO(KST:KPROF)*YDGSGEOM%GEMU(KST:KPROF)             &
       &                   +ZGMVFZ(KST:KPROF,JLEV)*YDGSGEOM%GSQM2(KST:KPROF)
    ENDDO
  ELSE
    ...
    ! Conversion
    DO JLEV=1,NFLEVG
      PUFZ(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM%GESLO(KST:KPROF)   &
       &                   +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM%GECLO(KST:KPROF)
      PVFZ(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)                             &
       &     *YDGSGEOM%GECLO(KST:KPROF)*YDGSGEOM%GEMU(KST:KPROF)              &
       &                   -ZGMVV(KST:KPROF,JLEV)                             &
       &     *YDGSGEOM%GESLO(KST:KPROF)*YDGSGEOM%GEMU(KST:KPROF)              &
       &                   +ZGMVZ(KST:KPROF,JLEV)*YDGSGEOM%GSQM2(KST:KPROF)
    ENDDO
  ENDIF
  ...
  ! Conversion
  DO JLEV=1,NFLEVG
    PGMVF(KST:KPROF,JLEV,YGP%MU)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM%GESLO(KST:KPROF)  &
       &                         +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM%GECLO(KST:KPROF)
...
IF(LDSLPHY) THEN
  ...
  ! Conversion
  DO JLEV=1,NFLEVG
    PUPF(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM%GESLO(KST:KPROF)  &
     &                   +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM%GECLO(KST:KPROF)
...
IF (YDDYNA%LNHDYN) THEN
  ...
  !     present only if NVDVAR=4 (the SLHD interpolation is never
  !     applied to this quantity):
  IF ((LL1.OR.LL3).AND.(YDDYNA%NVDVAR == 4)) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
