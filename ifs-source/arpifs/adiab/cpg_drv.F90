...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_DRV(YDGEOMETRY,YDMODEL,YDFIELDS,&
 !---------------------------------------------------------------------
 ! - INPUT .
 & CDCONF,LD_DFISTEP,YDSL,KGPCOMP,&
 & PGFLSLP,PGMVTNDHD_DDH,PGFLTNDHD_DDH,PGPSDT2D,PSD_PF,&
 !---------------------------------------------------------------------
 ! - INPUT/OUTPUT .
 & PPGTERM,&
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PGPSDT2D(YDGEOMETRY%YRDIM%NPROMA,YDMODEL%YRML_SPPT%YGPSDT(1)%NG2D,YDGEOMETRY%YRDIM%NGPBLKS)
...
ASSOCIATE(NDIM=>YGFL%NDIM, NDIM1=>YGFL%NDIM1, NDIMPC=>YGFL%NDIMPC, &
 ...
 & NCURRENT_ITER=>YDDYN%NCURRENT_ITER, NFLEVSF=>YDDYN%NFLEVSF, RSAVEDP=>YDDYN%RSAVEDP, &
 ...
 & SAVTEND=>YDSLPHY%SAVTEND,NVTEND=>YDSLPHY%NVTEND, &
...
LLSLPHY = LSLPHY.AND.YDDYNA%LSLAG.AND.(.NOT.LLCTC)
! Initial fluxes
IF (CDCONF == 'X') THEN
  LLFSTEP=.TRUE.
ENDIF

IF (YDDYNA%LSLAG .AND. ((NCURRENT_ITER == 0).OR.(LLCT.AND.(.NOT.LLCTC))) ) THEN
  LLUSEPB1=.TRUE.
ELSE
  LLUSEPB1=.FALSE.
ENDIF
...
DO JKGLO=1,KGPCOMP,NPROMA
  ...
  CALL CPG(YDGEOMETRY,YDMODEL,YDFIELDS,&
   ...
   & KGPCOMP,JKGLO,IBL,&
   & ZDT,ZDTPHY,ZTE,ZBETADT,YDSL,PGFLSLP(:,:,:,IBL),SAVTEND(:,:,:,IBL),&
   ...
   & PHIF0(:,:,IBL),PRCP0(:,:,:,IBL),PRE0F(:,:,IBL),PCTY0(:,:,:,IBL),PWRL9(:,:,IBL),RSAVEDP(:,:,:,IBL))
ENDDO
