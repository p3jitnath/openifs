...
REAL(KIND=JPRB),ALLOCATABLE :: ZGPPC(:,:)
...
LLUPDATE_NHX=YDDYNA%LNHX.AND.(YDDYNA%NVDVAR == 4).AND.((.NOT.YDDYNA%LGWADV).OR.(YDDYNA%LGWADV.AND.YDDYNA%ND4SYS==2.AND.NCURRENT_ITER<NSITER))
...
IF (YDDYNA%LPC_CHEAP) THEN
  IF (.NOT.ALLOCATED(ZGPPC)) ALLOCATE(ZGPPC(NPROMA,NFGPPC))
  ZGPPC(:,:)=0.0_JPRB
ENDIF
...
IF ( YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
  ...
  GPPCBUF (:, :, 1+(KSTGLO-1)/YDGEOMETRY%YRDIM%NPROMA) = ZGPPC(1:NPROMA,1:NFGPPC)
...
IF ( YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0) ) THEN
  ...
  ! fill interpolated quantities from buffer
  ! read gmv variables
  ZGPPC(1:NPROMA,1:NFGPPC) = GPPCBUF (:, :, 1+(KSTGLO-1)/YDGEOMETRY%YRDIM%NPROMA)
...
IF(YDDYNA%LGWADV) THEN
  ...
  ! * allocate ZVWVT1,ZVWVT1S and ZSVDT1.
  ...
  ! * deallocate arrays (zvwvt1,zvwvt1s,zsvdt1).
  IF (ALLOCATED (ZVWVT1)) DEALLOCATE(ZVWVT1)
  IF (ALLOCATED (ZVWVT1S)) DEALLOCATE(ZVWVT1S)
  IF (ALLOCATED (ZSVDT1)) DEALLOCATE(ZSVDT1)
...
IF ( ALLOCATED( ZGPPC ) ) DEALLOCATE(ZGPPC)
