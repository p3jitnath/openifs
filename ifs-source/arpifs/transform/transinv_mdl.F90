...
SUBROUTINE TRANSINV_MDL(YDGEOMETRY,YDGMV,YDMDDH,YDML_GCONF,YDDYNA, &
...
ENDIF
IENUV = YT0%NDIMUV
IF( YDDYNA%LGRADSP ) IENGMV = IENGMV-2
...
IF (LLHDDDH) THEN
  ...
  IF (NFTHER > 0) THEN
    ...
    ENDIF
    IF (YDDYNA%LNHX) ZSPTND_HV(1:NFLEVG,1:NSPEC2,4)= PSPTNDHD_SNHX(1:NFLEVG,1:NSPEC2)
    IF (NUMSPFLDS>0) THEN
      CALL INV_TRANS(&
