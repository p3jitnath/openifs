...
ASSOCIATE(NCMAX=>YDGEOMETRY%YRDIM%NCMAX, NDGENL=>YDGEOMETRY%YRDIM%NDGENL, NDGLG=>YDGEOMETRY%YRDIM%NDGLG, &
 ...
 & NSPSTAF=>YDGEOMETRY%YRMP%NSPSTAF, NSTA=>YDGEOMETRY%YRMP%NSTA, &
...
IF (LOUTPUT) THEN
  ...
  WRITE(NULOUT,*) "MAXIMUM NUMBER OF THREADS",OML_MAX_THREADS()
  WRITE(NULOUT,*) "CURRENT NUMBER OF THREADS",OML_MAX_THREADS()
  IF (NPRINTLEV >= 1) THEN
    WRITE(NULOUT,'('' NPROCM '')')
