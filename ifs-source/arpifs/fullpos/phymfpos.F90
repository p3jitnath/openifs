...
SUBROUTINE PHYMFPOS(YDQTYPE,YDNAMFPSCI,YDAFN,LDHPOS,YDGEOMETRY,YDDYNA,YDSURF,YDRIP,YDML_PHY_MF,KPROMA,KST,KND,KFLDAUX,KFLDSPP,KGFL, &
...
USE YOMCST   , ONLY : RV       ,RCPV     ,RETV     ,RCW      ,RCS      ,&
 & RLVTT    ,RLSTT    ,RTT      ,RALPW    ,RBETW    ,RGAMW    ,&
 & RALPS    ,RBETS    ,RGAMS    ,RALPD    ,RBETD    ,RGAMD    ,&
 & RKAPPA   ,RATM
...
REAL(KIND=JPRB) :: ZTBGOES11(KPROMA,4),ZTBGOES12(KPROMA,4),ZTBMTSAT1(KPROMA,4)
...
LLMTS = YDQTYPE%LL(TFP%MSAT7C1%ICOD) .OR. YDQTYPE%LL(TFP%MSAT7C2%ICOD) .OR.&
      ...
      & YDQTYPE%LL(TFP%MTSAT1C3%ICOD) .OR. YDQTYPE%LL(TFP%MTSAT1C4%ICOD)
LLMTS  =LLMTS.AND.LMTS
LL_MOCO=YDQTYPE%LL(TFP%MOCO%ICOD)
LLPBL  =LLSMC.OR.LLASMC.OR.LL_MOCO.OR.(LLICV.AND.(.NOT.LFPCAPEX)).OR.( &
 & (.NOT.LDHPOS).AND. &
...
! Nothing is done if (LLSMC,LLASMC,LLVSMC,LLICV,LLMTS,LLPBL)=(F,F,F,F,F)
IF (LLSMC.OR.LLASMC.OR.LLVSMC.OR.LLICV.OR.LLMTS.OR.LLPBL.OR.LLSRH) THEN
  ...
  ! GFL:
  IF (LLPBL .OR. LLICV .OR. LLMTS .OR. LLVSMC) THEN
    DO JLEV=1,NFLEVG
      IF (YDGFL%LLQ) THEN
        DO JROF=KST,KND
  ...
  IF (LLPBL.OR.LLICV.OR.LLMTS.OR.LLSRH) THEN
    ...
    CALL GPHPRE(KPROMA,NFLEVG,KST,KND,YDGEOMETRY%YRVAB,YDGEOMETRY%YRCVER,ZPRESH,PXYB=ZXYB,PRESF=ZPRESF)
  ...
  IF ((LLPBL).AND.(.NOT. YDARPHY%LMSE)) THEN
    ...
    CALL FPACHMT(YDML_PHY_MF,LLZ0H,KPROMA,KST,KND,NFLEVG,&
     & ZTT0,ZQT0,ZUT0,ZVT0,&
  ...
  IF (LLICV) THEN
    ...
    IF (LFPCAPEX) THEN
      ...
      ENDDO
      CALL FPCICA(YDML_PHY_MF,KST,KND,KPROMA,NFLEVG,ICAPE,RENTRA,PXTCLS,ZPCLS,PXRHCLS,ZTT0,ZPRESF,&
       & ZQT0,ZUSL_KAP,ZCAPE,ZCIN,ZTCVS,ILCL,IFCL,IEL)
    ELSE
      CALL FPCICA(YDML_PHY_MF,KST,KND,KPROMA,NFLEVG,ICAPE,RENTRA,ZTCLS,ZPCLS,ZRHCLS,ZTT0,ZPRESF,&
       & ZQT0,ZUSL_KAP,ZCAPE,ZCIN,ZTCVS,ILCL,IFCL,IEL)
    ENDIF
  ...
  IF (LL_MOCO) THEN
    ...
    ENDDO
    CALL ACCLPH(YDPHY0,YDPHY2,KST,KND,KPROMA,ITDIA,NFLEVG, &
  ...
  IF (LLMTS) THEN
    ...
    & ZTCLS,ZQCLS,&
    & ZTBMSAT7,ZTBMSAT8,ZTBMSAT9,ZTBGOES11,ZTBGOES12,ZTBMTSAT1)
