...
INTEGER(KIND=JPIM) :: IBL, IEND, IOFF, JKGLO, JROF, IPTR, JF, IJ, JL, IFLDIN, IFIELDS
...
IF (LFPOSBUF(YDFPGEOMETRY)) THEN
  ...
  CALL GSTATS(1434,0)
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JKGLO,IEND,IBL,JROF,IOFF,ZCORE,ZAUX,ZGPP)
  DO JKGLO=1,NGPTOT,NPROMA
    ...
    ELSE
      CALL VPOS(YDQTYPE,YDNAMFPSCI,YDAFN,LDHPOS,YDGEOMETRY%YRVAB,YDGEOMETRY,YDSURF,YDXFU,KCUFNR,PMCUFGP(:,:,IBL),YDMODEL%YRML_GCONF, &
...
ELSE
  ...
  ENDIF
  CALL GSTATS(1434,0)
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JKGLO,IEND,IBL,JROF,IOFF,ZAUX,ZGPP)
  DO JKGLO=1,NGPTOT,NPROMA
    IEND=MIN(NPROMA,NGPTOT-JKGLO+1)
    IBL=(JKGLO-1)/NPROMA+1
    IF (NFPGT1 > 0) THEN
