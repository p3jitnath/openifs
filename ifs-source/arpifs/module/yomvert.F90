...
TYPE TVFE
  ...
  REAL(KIND=JPRD),ALLOCATABLE :: RDDERBF11(:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RINTBF11_IMPL (:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDERBF01_IMPL (:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDERBF10_IMPL (:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDERBF11_IMPL (:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDERBH01_IMPL (:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDDERBF01_IMPL(:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDDERBF10_IMPL(:,:)
  REAL(KIND=JPRD),ALLOCATABLE :: RDDERBF11_IMPL(:,:)
...
!*       1.3   VFE INTEGRAL AND DERIVATIVE OPERATORS
    ...
    ENDIF
    IF (.NOT.FROM%YRCVER%LVFE_ECMWF) THEN
      ...
      CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDDERBF11 ,  FROM%YRVFE%RDDERBF11 )
      IF (FROM%LNONHYD_GEOM) THEN
        ...
        ENDIF
        IF ((FROM%YRCVER%LVFE_LAPL.AND.FROM%YRCVER%LVFE_LAPL_HALF).OR.FROM%YRCVER%LVFE_DELNHPRE) THEN
          ...
          IF (FROM%YRCVER%NVFE_DERBC==2) THEN
            CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDERBH01_IMPL  , FROM%YRVFE%RDERBH01_IMPL  )
          ENDIF
        ENDIF
        IF (FROM%YRCVER%NVFE_DERBC==2) THEN
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDERBF01_IMPL  , FROM%YRVFE%RDERBF01_IMPL  )
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDERBF10_IMPL  , FROM%YRVFE%RDERBF10_IMPL  )
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDERBF11_IMPL  , FROM%YRVFE%RDERBF11_IMPL  )
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDDERBF01_IMPL , FROM%YRVFE%RDDERBF01_IMPL )
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDDERBF10_IMPL , FROM%YRVFE%RDDERBF10_IMPL )
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RDDERBF11_IMPL , FROM%YRVFE%RDDERBF11_IMPL )
        ENDIF
        IF (FROM%YRCVER%NVFE_INTBC==2) THEN
          CALL D_ALLOC_SIZEOF_2D(THIS%YRVFE%RINTBF11_IMPL  , FROM%YRVFE%RINTBF11_IMPL  )
        ENDIF
      ENDIF
    ENDIF
  ENDIF
...
SUBROUTINE DEALLOC_VERTICAL_GEOM( THIS )
  ...
  IF (ALLOCATED(THIS%YRVFE%RDDERBF11)) DEALLOCATE(THIS%YRVFE%RDDERBF11 )
  IF (ALLOCATED(THIS%YRVFE%RINTBF11_IMPL)) DEALLOCATE(THIS%YRVFE%RINTBF11_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDERBF01_IMPL)) DEALLOCATE(THIS%YRVFE%RDERBF01_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDERBF10_IMPL)) DEALLOCATE(THIS%YRVFE%RDERBF10_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDERBF11_IMPL)) DEALLOCATE(THIS%YRVFE%RDERBF11_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDERBH01_IMPL)) DEALLOCATE(THIS%YRVFE%RDERBH01_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDDERBF01_IMPL)) DEALLOCATE(THIS%YRVFE%RDDERBF01_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDDERBF10_IMPL)) DEALLOCATE(THIS%YRVFE%RDDERBF10_IMPL)
  IF (ALLOCATED(THIS%YRVFE%RDDERBF11_IMPL)) DEALLOCATE(THIS%YRVFE%RDDERBF11_IMPL)
