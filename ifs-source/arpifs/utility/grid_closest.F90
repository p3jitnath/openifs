...
!    Purpose.
!    --------
!      Closest point interpolation of grid field from model resolution
...
REAL(KIND=JPRB) :: ZLAT, ZLON, ZDLON, ZDLONN, ZDLONS, ZWGTLAT, ZWGTNW, ZWGTSW, &
 & ZLONNW, ZLONSW  
REAL(KIND=JPRB) :: ZWSE,ZWSW,ZWNE,ZWNW,ZMAX
...
IF (LHOOK) CALL DR_HOOK('GRID_CLOSEST',0,ZHOOK_HANDLE)
...
DO JLAT=1,KLAT_OUT
  ...
  DO JLON=1,KLON_OUT(JLAT)
    ...
    ZWSE = ZWGTLAT*ZWGTSW
    ZWSW = ZWGTLAT*(1.0-ZWGTSW)
    ZWNE = (1.0-ZWGTLAT)*ZWGTNW
    ZWNW = (1.0-ZWGTLAT)*(1.0-ZWGTNW)

    ZMAX = MAX(ZWSE,ZWSW,ZWNE,ZWNW)
    IF(ZWSE == ZMAX) THEN
      PFLD_OUT(JLON+IND_OUT(JLAT)) = PFLD_IN(IND_IN(ILATS)+ISE)
    ELSEIF(ZWSW == ZMAX) THEN
      PFLD_OUT(JLON+IND_OUT(JLAT)) = PFLD_IN(IND_IN(ILATS)+ISW)
    ELSEIF(ZWNE == ZMAX) THEN
      PFLD_OUT(JLON+IND_OUT(JLAT)) = PFLD_IN(IND_IN(ILATN)+INE)
    ELSE
      PFLD_OUT(JLON+IND_OUT(JLAT)) = PFLD_IN(IND_IN(ILATN)+INW)
    ENDIF
  ENDDO
ENDDO
!$OMP END DO
...
IF (LHOOK) CALL DR_HOOK('GRID_CLOSEST',1,ZHOOK_HANDLE)
