...
!**** *GWDRAGWMS_S_LAYER* - Routine computing the simplified non orographic gravity wave drag
...
IF (LHOOK) CALL DR_HOOK('GWDRAGWMS_S_LAYER',0,ZHOOK_HANDLE)
ASSOCIATE(NDIM=>YGFL%NDIM, YNOGW=>YGFL%YNOGW, &
 & GTPHYGWWMS=>YDEGWWMS%GTPHYGWWMS)
!     ------------------------------------------------------------------


!*         1.     UNROLL THE DERIVED STRUCTURES AND CALL GWDRAG_WMSS

CALL GWDRAG_WMSS( YDEGWD,YDEGWWMS,  KDIM%KIDIA,  KDIM%KFDIA, KDIM%KLON  , KDIM%KLEV , GTPHYGWWMS, &
  & STATE%T, STATE%U, STATE%V, PAUX%PAPRSF, PAUX%PAPRS, PAUX%PAPHIF, &
  & PAUX%PGELAT,  PAUX%PGAW,& 
  & NOGW(1)%P, NOGW(2)%P, ZFLXWGWU, ZFLXWGWV )
...
!     ------------------------------------------------------------------
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('GWDRAGWMS_S_LAYER',1,ZHOOK_HANDLE)
