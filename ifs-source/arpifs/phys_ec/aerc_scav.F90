...
!*** * AERC_SCAV* - IN-CLOUD AND BELOW CLOUD SCAVENGING OF TRACERS
...
!**   INTERFACE.
!     ----------
!          *AERC_SCAV* IS CALLED FROM *AER_PHY3*.
!
!     AUTHOR.
!     -------
!        Johannes Flemming 
...
REAL(KIND=JPRB) :: ZBETA,  ZBETAR, ZBETAC
...
REAL(KIND=JPRB)   :: ZWLOSS(KLON,KAER), ZWGAIN(KLON,KAER)
REAL(KIND=JPRB)   :: ZSCAV_COEF
...
!-----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('AERC_SCAV',0,ZHOOK_HANDLE)
...
!evaporation
    ...
    ZBETAC = PCONDN(JL,JK)+PCONDL(JL,JK)
    ZCLWAT =  PCLWAT(JL,JK)
    ZCLICE =  PCLICE(JL,JK)

    IF (  ZBETAC ==  0.0_JPRB )  CYCLE
!    IF ( LLCHEM_WDFR ) THEN
!* use effective flux and cloud cover for liquid water content
      ...
      IF (ZCLCOV >= ZMINCLCOV) THEN
        ...
        ZCONDL =  MAX(0.0_JPRB,PCONDL(JL,JK) /  ZCLCOV)
        ZCONDI =  MAX(0.0_JPRB,PCONDN(JL,JK) /  ZCLCOV)
        ZBETAC =  MAX(0.0_JPRB,ZBETAC /  ZCLCOV)
      ELSE
        ZCLWAT = 0.0_JPRB
        ZCLICE = 0.0_JPRB
        ZCONDL = 0.0_JPRB
...
!* relative water gain due to condensation
        IF (ZCLTOT > ZEPSQLIQ .AND. ZCONDL+ZCONDI > ZEPSFLX) THEN
            ZSCAV_COEF =  YAERO_DESC(JT)%RSCAVIN
! Bourgeois and Bey, JGR 2011
         IF (PTP(JL,JK) < 238.15_JPRB) THEN
! ice cloud
           ZSCAV_COEF=0.06_JPRB
         ELSEIF (PTP(JL,JK) < 273.15_JPRB) THEN
! mixed cloud
           IF (LLCONV) THEN
! no info on water and ice content
             ZSCAV_COEF=ZSCAV_COEF*(PTP(JL,JK)-238.15_JPRB)/35._JPRB+0.06_JPRB*(273.15_JPRB-PTP(JL,JK))/35._JPRB
           ELSE
             ZSCAV_COEF=ZSCAV_COEF*PCLWAT(JL,JK)/(PCLICE(JL,JK)+PCLWAT(JL,JK)) &
               &        +0.06_JPRB*PCLICE(JL,JK)/(PCLICE(JL,JK)+PCLWAT(JL,JK))
           ENDIF
         ENDIF

           ZBETAR=ZBETAC / ( ZDP * ZCLTOT / RG ) ! in s-1
!* safety check
...
!* scavenging coefficient  in s-1 
 !In-cloud scavenging is different for aerosol than for gas-phase 
        IF ( LLWDAER ) THEN
!         ! aerosol: use interstitial fraction 
...
!
        ...
        ZDX = (PCEN(JL,JK,JT)+PTENC0(JL,JK,JT)*PTSPHY)*(ZFUNC - 1._JPRB) * ZCLCOV      ! in kg kg-1
        !IF (JT==3) THEN
        !   write (*,*) "AERC_SCAV SCAVIN",LLCONV,ZCLCOV,ZPRCOV,ZBETA,ZBETAR, &
        !     & ZSCAV,ZFUNC,PCEN(JL,JK,JT),ZDX,PTENC1(JL,JK,JT),ZDX/PTSPHY
        !ENDIF
      ...
      !* rain reevaporation  ! bottom flux smaller and top flux above limit
...
!  ! eq 17  minus delta m_i_top
          ...
          ENDIF
        !IF (JT==3) THEN
        !   write (*,*) "AERC_SCAV SCAVBC",LLCONV,ZCLCOV,ZPRCOV,ZBETA,ZBETAR &
        !     & ZSCAV,ZFUNC,PCEN(JL,JK,JT),ZDX,PTENC1(JL,JK,JT),ZDX/PTSPHY
        !ENDIF
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('AERC_SCAV',1,ZHOOK_HANDLE)
