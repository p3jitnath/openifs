...
!**** *PP2DINT* - LINEARLY INTERPOLATE FIELDS TO GIVEN LEVEL
!               WITH LINEAR EXTRAPOLATION AT TOP AND SURFACE VALUE AT
!               BOTTOM
...
!        KHILEV                    - END FOR INTERPOLATION            (INPUT)
...
!     AUTHOR.
!     -------
!      C. PERIARD A. JOLY, based on PPINTP
!      ORIGINAL : 27/5/92
...
!*       1.1   INTERPOLATE .
IF (LHOOK) CALL DR_HOOK('PP2DINT',0,ZHOOK_HANDLE)

DO JLEVP=KLOLEV,KHILEV
  IF (.NOT.LDBLOW(JLEVP)) THEN
    DO JL=KSTART,KPROF
      IBL=MAX(2,KLEVB(JL,JLEVP,KSLCT))
      IAB=IBL-1
      PFLDO(JL,JLEVP)=PFLDI(JL,IBL)+(PFLDI(JL,IBL)-PFLDI(JL,IAB))* &
       & (PRPRESC(JL,JLEVP)-PRXP(JL,IBL,KSLCT))*PRXPD(JL,IAB,KSLCT)  
    ENDDO
  ELSE
    DO JL=KSTART,KPROF
      IF (LDBELO(JL,JLEVP)) THEN
        PFLDO(JL,JLEVP) = PFLDI(JL,KFLEV)
      ELSE
        IBL=MAX(2,KLEVB(JL,JLEVP,KSLCT))
        IAB=IBL-1
        PFLDO(JL,JLEVP)=PFLDI(JL,IBL)+(PFLDI(JL,IBL)-PFLDI(JL,IAB))* &
...
IF (LHOOK) CALL DR_HOOK('PP2DINT',1,ZHOOK_HANDLE)
