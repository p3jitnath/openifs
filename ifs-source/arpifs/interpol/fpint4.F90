...
!**** *FPINT4*  - FULL-POS horizontal interpolator.
...
!     METHOD.
!     -------
!        Horizontal interpolations : with 4 or the nearest point(s).
...
!      ORIGINAL : 98-09-07 from FPINT12
...
INTEGER(KIND=JPIM) :: IADD(KFPROMA), INRST(KFPROMA)
INTEGER(KIND=JPIM) :: IADDFLD, IMAX0(KFPROMA,1)

INTEGER(KIND=JPIM) :: JDER, JF, JI, IFLDBUF
LOGICAL :: LLIZNA
...
!     -----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('FPINT4',0,ZHOOK_HANDLE)
!     -----------------------------------------------------------------------

IF (KFPROW < 4) CALL ABOR1('FPINT4 : KFPROW TOO SMALL') ! because it supports also the multilinear interpolations
...
IF (ANY(LDNRST)) THEN
  ...
  DO JI=KGPST,KGPEND
    IMAX0(JI,1:1)=MAXLOC(PWXX(JI,1:4))
  ENDDO
  DO JI=KGPST,KGPEND
    SELECT CASE (IMAX0(JI,1))
    CASE (1)
      INRST(JI)=KL0(JI,2)+1
    CASE (2)
      INRST(JI)=KL0(JI,2)+2
...
DO JF = 1, KFIELDS
  ...
  IF (.NOT.LDMASK(JF)) THEN
    ...
    IF (.NOT.LDNRST(JF)) THEN
      IF (LLIZNA) THEN
        DO JI=KGPST,KGPEND
          IF (ANY (PWXX(JI,1:4) > 0._JPRB)) THEN
            PROW(JI,JF)=&
    ...
    ELSE
      DO JI=KGPST,KGPEND
        PROW(JI,JF) = PBUF(IADD(JI)+INRST(JI))
      ENDDO
    ENDIF
  ...
  IF (IADDFLD > KFLDBUF) THEN
    CALL ABOR1('FPINT4 : INTERNAL ERROR IADDFLD')
  ENDIF

ENDDO

!     -----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('FPINT4',1,ZHOOK_HANDLE)
