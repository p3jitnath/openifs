...
!****-------------------------------------------------------------------
!**** *SLCOMM* - Interpolation buffer communication
...
!                           These are always sent for all levels.
!                           KFIXFLD(1)=index into interpolation buffer for the
!                                      first fixed field
!                           KFIXFLD(2)=index into interpolation buffer for the
!                                      last level of the last fixed field
!          LDINC          - increment flag for interpolation buffer
!          LD_TLEVOL      - switch for tl evolution of perturbation run
!          KTYP           - type of communication.
!                           0 = cf. old CY35 version of SLCOMM
!                           1 = cf. old CY35 version of SLCOMM1
...
!     SLCOMM and are sending us data. By probing for messages we avoid 
...
LOGICAL            :: LLSLCOMM(YDSL%NSLPROCS), LLSWITCH
...
!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('SLCOMM',0,ZHOOK_HANDLE)
!     ------------------------------------------------------------------

!     1/ Preliminary calculations.

IF(PRESENT(LDSLCOMM))THEN
  IF (.NOT. PRESENT(LD_TLEVOL) ) THEN
    CALL ABOR1('Calling SLCOMM with LDSLCOMM and no LD_TLEVOL is not allowed')
  ENDIF
  IF((GET_NSIM4D() > 0).AND.(.NOT. LD_TLEVOL)) THEN
    LLSLCOMM(:)=LDSLCOMM(:)
  ELSE
    LLSLCOMM(:)=.TRUE.
  ENDIF
  IF( LSLADREP ) LLSLCOMM(:)=.TRUE.
ELSE
  LLSLCOMM(:)=.TRUE.
ENDIF
...
I_MSENDTOT=0
I_MRECVTOT=0
INUMFIELDS=IFIXFLD(2)-IFIXFLD(1)+1
...
IF( .NOT.LLSWITCH )THEN
  ...
  CALL SLCOMM_INT(YDSL,YDSL%NSLRPT,YDSL%NSLSPT,YDSL%NSLRECVPOS,YDSL%NSLSENDPOS,&
                 ...
                 & IFIXFLD,KFLDSLB,LDINC,KTYP,PB1A)

ELSE

  DO J=1,YDSL%NSLPROCS
    IRECVTOT(J)=YDSL%NSLSENDPTR(J+1)-YDSL%NSLSENDPTR(J)
    ISENDTOT(J)=YDSL%NSLRECVPTR(J+1)-YDSL%NSLRECVPTR(J)
  ENDDO
  DO J=1,YDSL%NSLPROCS
    IF(ISENDTOT(J) > 0 ) THEN
      IJS=IJS+1
  ...
  CALL SLCOMM_INT(YDSL,YDSL%NSLSPT,YDSL%NSLRPT,YDSL%NSLSENDPOS,YDSL%NSLRECVPOS,&
                 & YDSL%NSLSENDPTR,YDSL%NSLRECVPTR,&
                 & IFIXFLD,KFLDSLB,LDINC,KTYP,PB1A)
ENDIF


IF (LHOOK) CALL DR_HOOK('SLCOMM',1,ZHOOK_HANDLE)

CONTAINS

SUBROUTINE SLCOMM_INT(YDSL,KSLRPT,KSLSPT,KSLRECVPOS,KSLSENDPOS,&
 & KSLRECVPTR,KSLSENDPTR,KFIXFLD,KFLDSLB,LDINC,KTYP,PB1A)
...
REAL(KIND=JPRB), ALLOCATABLE :: ZSENDBUF(:) ! 0:I_MSENDTOT*INUMFIELDS
REAL(KIND=JPRB), ALLOCATABLE :: ZRECVBUF(:) ! 0:I_MRECVTOT*INUMFIELDS
...
IF (LHOOK) CALL DR_HOOK('SLCOMM:SLCOMM_INT',0,ZHOOK_HANDLE)

ALLOCATE(ZSENDBUF(0:I_MSENDTOT*INUMFIELDS))
ALLOCATE(ZRECVBUF(0:I_MRECVTOT*INUMFIELDS))
...
ELSEIF (KTYP == 1) THEN
  CALL GSTATS_BARRIER(757)
ENDIF
CALL GSTATS(509,0)

IF (LSYNC_SLCOM) THEN
  CALL MPL_BARRIER(CDSTRING='SLCOMM:SLCOMM_INT')
ENDIF
...
DO J=1,YDSL%NSLPROCS
  IF( IRECVTOT(J) > 0 .AND. LLSLCOMM(J) )THEN
    ...
    CALL MPL_RECV(ZRECVBUF(IRECV_POS(J):IRECV_POS(J)+ILEN),&
     ...
     & KTAG=ITAG,CDSTRING='SLCOMM:SLCOMM_INT RECV' )  
  ENDIF
ENDDO
...
DO J=1,YDSL%NSLPROCS
  IF ( ISENDTOT(J) > 0 .AND. LLSLCOMM(J) )THEN
    ...
    ILEN=IEND-IBEG+1
    IJPOS(J)=(IFIXFLD(2)-IFIXFLD(1)+1)*ILEN
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,I_KPOS,JJ)
    DO JFLD=IFIXFLD(1),IFIXFLD(2)
      I_KPOS=ISEND_POS(J)+(JFLD-IFIXFLD(1))*ILEN
      DO JJ=IBEG,IEND
        ZSENDBUF(I_KPOS+JJ-IBEG+1)=PB1A(KSLSENDPOS(JJ),JFLD)
      ENDDO
    ENDDO
!$OMP END PARALLEL DO
    IRSND=IRSND+1
    CALL MPL_SEND(ZSENDBUF(ISEND_POS(J):ISEND_POS(J)+IJPOS(J)),&
     ...
     & KTAG=ITAG,CDSTRING='SLCOMM:SLCOMM_INT SEND' )
  ENDIF
ENDDO
...
IF (LDINC) THEN
  ...
  DO JNR=1,IRRCV
    CALL MPL_WAIT(KREQUEST=IREQ_RCV(JNR),CDSTRING='SLCOMM:SLCOMM_INT WAIT FOR RECVS')
    J=IREQINDX(JNR)
    IBEG=KSLRECVPTR(J)
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,I_KPOS,JJ)
    DO JFLD=IFIXFLD(1),IFIXFLD(2)
      I_KPOS=IRECV_POS(J)+(JFLD-IFIXFLD(1))*ILEN
      DO JJ=IBEG,IEND
        PB1A(KSLRECVPOS(JJ),JFLD)=PB1A(KSLRECVPOS(JJ),JFLD)+ZRECVBUF(I_KPOS+JJ-IBEG+1)
      ENDDO
    ENDDO
!$OMP END PARALLEL DO
  ENDDO
ELSE
  DO JNR=1,IRRCV
    CALL MPL_WAITANY(KREQUEST=IREQ_RCV(1:IRRCV),KINDEX=INR,CDSTRING='SLCOMM:SLCOMM_INT: WAIT FOR ANY RECV')
    J=IREQINDX(INR)
    IBEG=KSLRECVPTR(J)
    IEND=KSLRECVPTR(J)+ZRECVBUF(IRECV_POS(J))-1
    ILEN=IEND-IBEG+1
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,I_KPOS,JJ)
    DO JFLD=IFIXFLD(1),IFIXFLD(2)
      I_KPOS=IRECV_POS(J)+(JFLD-IFIXFLD(1))*ILEN
      DO JJ=IBEG,IEND
        PB1A(KSLRECVPOS(JJ),JFLD)=ZRECVBUF(I_KPOS+JJ-IBEG+1)
      ENDDO
    ENDDO
!$OMP END PARALLEL DO
  ENDDO
...
IF(IRSND > 0) THEN
  CALL MPL_WAIT(KREQUEST=IREQ_SND(1:IRSND),CDSTRING='SLCOMM:SLCOMM_INT WAIT FOR SENDS')
ENDIF

DEALLOCATE(ZRECVBUF)
DEALLOCATE(ZSENDBUF)

IF (LHOOK) CALL DR_HOOK('SLCOMM:SLCOMM_INT',1,ZHOOK_HANDLE)
