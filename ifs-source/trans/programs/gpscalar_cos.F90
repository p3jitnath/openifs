...
REAL(KIND=JPRB),ALLOCATABLE :: ZSPEC(:,:),ZFPDAT(:),ZSPECG(:,:),ZSPECM(:,:)
REAL(KIND=JPRB),ALLOCATABLE :: ZG(:,:,:), ZGG(:,:), ZGGOUT(:),ZGGM(:,:,:)
LOGICAL :: LLSCAL, LLATLON_OUT,LLATLON_IN, LLSPECIN,LLLL

INTEGER(KIND=JPIM) :: IPOI, JL, JGL, J, IR
REAL(KIND=JPRB)    :: ZDEG2RAD, ZLON, ZLONI
REAL(KIND=JPRB)    :: RPI, RA
TYPE(C_PTR)        :: MEMBUF
...
!ZDLAT = ZDTOR*REAL(ISP-INP,JPRB)/REAL(ILATS-1,JPRB)
...
IF( .NOT.LLSPECIN ) THEN
  IF( MYPROC == 1 ) THEN
    IF( ITEST == 1 ) THEN
      ...
      IPOI=0
      ZDEG2RAD=RPI/180._JPRB
      DO JGL=1,NDGLI
        ZLONI=360._JPRB/FLOAT(NLOEN(JGL))*ZDEG2RAD
        DO JL=1,NLOEN(JGL)
          IPOI=IPOI+1
  ...
  IF(MYPROC == 1) THEN
    ...
    CALL GRIB_SET(IGRIB_OUT,'laplacianOperator',0.5_JPRB)
    IF(NSMAX>= 213) THEN
      ISTRUNC = 20
    ELSE
