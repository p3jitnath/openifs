...
contains
  ...
  subroutine generate_column_exp_exp(ng, nlev, ig, random_stream, pdf_sampler, &
    ...
    ! Sample from a lognormal or gamma distribution to obtain
    ! the optical depth scalings
    ! call pdf_sampler%sample(fractional_std(itrigger:iend), &
    ...
    ! Since with the Exp-Exp scheme we do all clouds in a profile
    ! together, some of these layers may not be cloudy
    ! Moved to upper loop
    ! where (.not. is_cloudy) 
    !   od_scaling(ig,:) = 0.0_jprb
    ! end where
